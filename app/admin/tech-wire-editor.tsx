...
type FormState = { id?: string; title: string; slug: string; category: string; summary: string; readTime: string; content: string; pros: string; cons: string; sourceLabel: string; sourceUrl: string };
const emptyForm: FormState = { title: "", slug: "", category: "", summary: "", readTime: "5 min read", content: "", pros: "", cons: "", sourceLabel: "", sourceUrl: "" };
...
function toForm(article: DatabaseTechArticle): FormState {
  ...
  return { id: article.id, title: article.title, slug: article.slug, category: article.category, summary: article.summary, readTime: article.read_time, content: main, pros: (pros?.bullets || []).join("\n"), cons: (cons?.bullets || []).join("\n"), sourceLabel: article.sources[0]?.label || "", sourceUrl: article.sources[0]?.url || "" };
}
...
export default function TechWireEditor({ initialArticles, userId }: { initialArticles: DatabaseTechArticle[]; userId: string }) {
  ...
  async function save(status: "draft" | "published") {
    ...
    const values = { slug, category: form.category.trim(), title: form.title.trim(), summary: form.summary.trim(), read_time: form.readTime.trim() || "5 min read", sections, sources, status, published_at: status === "published" ? new Date().toISOString() : null, updated_at: new Date().toISOString() };

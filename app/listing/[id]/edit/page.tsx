...
export default async function EditListingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase.from("listings").select("id,title,description,price,condition,category,location,trade,image_url,image_urls,video_url").eq("id", id).eq("user_id", user.id).maybeSingle();
  if (!data) notFound();

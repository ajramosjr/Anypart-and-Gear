...
export default function ManageListing({ id, status }: { id: string; status: string }) {
  ...
  return <div className="manage-actions"><button disabled={busy} onClick={() => update(status === "sold" ? "active" : "sold")}>{status === "sold" ? "Mark active" : "Mark sold"}</button><button className="danger" disabled={busy} onClick={() => update("remove")}>Remove</button></div>;
}

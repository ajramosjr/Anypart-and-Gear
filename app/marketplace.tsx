...
export default function Marketplace({ user, signInPath, signOutPath, listings }: { user: { id: string; name: string; email: string } | null; signInPath: string; signOutPath: string; listings: Listing[] }) {
  ...
  return <main className="mechanical-shell min-h-screen bg-[#eef1f4] text-slate-950">
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 text-slate-900 backdrop-blur">
      ...
      {mobileNav && <nav className="grid gap-1 border-t border-slate-200 px-4 py-3 text-sm font-semibold lg:hidden"><a className="rounded-lg p-3 hover:bg-slate-100" href="#listings">Browse listings</a><a className="rounded-lg p-3 hover:bg-slate-100" href="#categories">Categories</a><Link className="rounded-lg p-3 hover:bg-slate-100" href="/parts-wanted">Parts Wanted</Link><Link className="rounded-lg p-3 hover:bg-slate-100" href="/shops">Local shops</Link><Link className="rounded-lg p-3 hover:bg-slate-100" href="/tech-wire">APG Tech Wire</Link><a className="rounded-lg p-3 hover:bg-slate-100" href="/safety">Safety tips</a><a className="rounded-lg p-3 hover:bg-slate-100" href="#business">For businesses</a>{user?<><a className="rounded-lg p-3 hover:bg-slate-100" href="/account">My account</a><a className="rounded-lg p-3 hover:bg-slate-100" href="/messages">Messages</a><a className="rounded-lg p-3 hover:bg-slate-100" href="/notifications">Notifications</a><a className="rounded-lg p-3 text-red-700 hover:bg-red-50" href={signOutPath}>Sign out</a></>:<a className="rounded-lg border border-amber-500 bg-amber-400 p-3 text-center font-extrabold text-[#071a35] shadow-sm hover:bg-amber-300" href={signInPath}>Sign in or create account</a>}</nav>}
    </header>
    ...
    <section className="mx-auto max-w-7xl px-4 pb-14 sm:px-6" aria-labelledby="parts-wanted-home-title"><div className="grid overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-sm lg:grid-cols-[1.1fr_.9fr]"><div className="p-7 sm:p-9"><p className="eyebrow">Can’t find it?</p><h2 id="parts-wanted-home-title" className="section-title">Tell local businesses what you need.</h2><p className="mt-4 max-w-xl text-base leading-7 text-slate-600">Post a Parts Wanted request—even if you don’t know the part’s name. Add your year, make, model, description and photos so verified businesses can help.</p><div className="mt-6 flex flex-col gap-3 sm:flex-row"><Button asChild className="gold-button h-11 px-6 font-black"><Link href={user?"/parts-wanted/new":"/login?next=/parts-wanted/new"}><Search className="size-4"/> Request a part</Link></Button><Button asChild variant="outline" className="h-11 px-6 font-bold"><Link href="/parts-wanted"><CircleHelp className="size-4"/> How it works</Link></Button></div></div><div className="flex flex-col justify-center bg-[#0b2345] p-7 text-white sm:p-9"><p className="text-sm font-black uppercase tracking-[.14em] text-amber-400">For verified businesses</p><h3 className="mt-3 text-2xl font-black">Real requests from local buyers.</h3><p className="mt-3 leading-7 text-slate-300">Respond when you have the part or can help identify it—without uploading your entire inventory.</p><Link href="/shops/register" className="mt-5 inline-flex items-center gap-2 font-bold text-amber-300">Register your business <ChevronRight className="size-4"/></Link></div></div></section>

...
export default function Marketplace({ user, signInPath, signOutPath, listings }: { user: { name: string; email: string } | null; signInPath: string; signOutPath: string; listings: Listing[] }) {
  ...
  return <main className="mechanical-shell min-h-screen bg-[#eef1f4] text-slate-950">
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 text-slate-900 backdrop-blur">
      ...
      {mobileNav && <nav className="grid gap-1 border-t border-slate-200 px-4 py-3 text-sm font-semibold lg:hidden"><a className="rounded-lg p-3 hover:bg-slate-100" href="#listings">Browse listings</a><a className="rounded-lg p-3 hover:bg-slate-100" href="#categories">Categories</a><a className="rounded-lg p-3 hover:bg-slate-100" href="/safety">Safety tips</a><a className="rounded-lg p-3 hover:bg-slate-100" href="#business">For businesses</a>{user?<><a className="rounded-lg p-3 hover:bg-slate-100" href="/account">My account</a><a className="rounded-lg p-3 hover:bg-slate-100" href="/messages">Messages</a><a className="rounded-lg p-3 text-red-700 hover:bg-red-50" href={signOutPath}>Sign out</a></>:<a className="rounded-lg bg-[#0b2345] p-3 text-center text-white" href={signInPath}>Sign in or create account</a>}</nav>}
    </header>

...
type Shop = { name: string; specialty: string; description: string; location: string; postal_code: string; hours: string; website: string | null; services: string[] };
...
export default function ShopForm({ userId, shop }: { userId: string; shop: Shop | null }) {
  ...
  return <form className="listing-form" onSubmit={submit}>
    <div className="form-grid">
      ...
      <div className="field"><label htmlFor="website">Website (optional)</label><input id="website" name="website" type="url" defaultValue={shop?.website || ""} placeholder="https://example.com" /></div>
    ...
    <div className="form-actions"><button className="button" disabled={saving}>{saving ? "Saving..." : shop ? "Update business" : "Add business"}</button>{message && <span className="form-message error">{message}</span>}</div>
  </form>;
}
